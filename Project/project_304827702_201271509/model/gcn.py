...
class HyperGCNConv(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, improved=False, cached=False, bias=True, normalize=True,
                 use_hypernetworks=False, **kwargs):
        super(HyperGCNConv, self).__init__(aggr='add', **kwargs)
        ...
        self.weight = nn.Parameter(torch.Tensor(in_channels, out_channels)) if not self.use_hypernetworks else None
        self.W_hyper = None
    ...
    def set_hypernetworks_weights(self, W_hyper=None):
        if self.use_hypernetworks and torch.is_tensor(W_hyper):
            self.W_hyper = W_hyper

    def forward(self, x, edge_index, edge_weight=None):
        """"""
        if self.use_hypernetworks and torch.is_tensor(self.W_hyper):
